...
# CLASSPATH_DIR's jars, then r8 shrinks+dexes the result using PG_CONF, then the resulting
# classes.dex is embedded into <target> via the existing konative_embed_binary_blob() (.incbin,
# build-time regeneration, optional SHA-256 self-check - see KonativeEmbedBlob.cmake). The actual
# compile+dex work happens at BUILD time in KonativeCompileKotlinDex.cmake (a `cmake -P` driver,
# same shape as KonativeGenerateIncbinAsm.cmake), not here at configure time - this file only wires
# up the add_custom_command/add_custom_target graph.
#
...
#
# Requires three machine-local toolchain cache variables/environment variables, following the exact
...
function(konative_embed_kotlin_dex TARGET_NAME)
  cmake_parse_arguments(ARG "VERIFY_SHA256" "CLASSPATH_DIR;PG_CONF;SYMBOL;MIN_API" "SOURCES" ${ARGN})
  ...
  add_custom_command(
    ...
    DEPENDS ${ARG_SOURCES} "${ARG_PG_CONF}" "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/KonativeCompileKotlinDex.cmake"
    COMMENT "konative: compiling+dexing ${ARG_SYMBOL} (kotlinc -> r8) from ${CMAKE_CURRENT_FUNCTION_LIST_DIR}"
    VERBATIM)
