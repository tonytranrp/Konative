...
- **`SystemSequence` is intentionally single-threaded and dumb.** Parallel/cross-system-dependency
  ...
  or a task graph directly into `SystemSequence`/`system.hpp`. If a system needs to run across
...
`SystemSequence`-adjacent scheduling primitive, a new `World` accessor). Concrete gameplay
