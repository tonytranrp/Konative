...
| Real desktop CI (`.github/workflows/desktop-tests.yml`) | **Landed and verified — and it immediately caught 2 real, previously-hidden bugs, then a 3rd later (see the Android CI row below).** This project's docs said things were "verified in CI" for a long time before any CI actually existed (`.github/workflows/` was empty before 2026-07-22). The first real run found `tests/test_result.cpp` used `std::string` with no `#include <string>` — silently working on this dev machine's Clang/libc++ toolchain (transitively included) but failing outright on GCC/Linux (fixed). The second real run found an actual segfault in `NextEventAwaiter`'s core value-returning path on GCC — see its own row above and `next_event_awaiter.hpp`'s doc comment; guarded to Clang-only rather than silently left broken. Runs the `desktop-debug` preset only — the Android presets are covered by a separate, real workflow, not this one (see below) |
| Real Android CI, build leg (`.github/workflows/android-build.yml`, `android-build` job) | **Landed and verified — a real hosted-runner Android build needing no machine-local `CMakeUserPresets.json` at all.** Matrix-builds both `android-arm64`/`android-x86_64` CMake presets: a real SDK (`android-actions/setup-android`)/NDK (`nttld/setup-ndk`)/kotlinc (downloaded + SHA-256-verified straight from JetBrains' GitHub releases) set up fresh on the runner every time (not assumed pre-installed), and `tools/kotlin-classpath-resolver/` (§9) resolving the AndroidX/Compose/coroutines classpath in place of the machine-local, gitignored directories `CMakeUserPresets.json` points at locally. Getting this green took 5 real, iterative CI-only fixes, none reproducible on this project's own Windows/Clang dev machine: a wrong NDK version-string format for `nttld/setup-ndk`; an unsorted-`file(GLOB)` bug in `KonativeEmbedKotlinDex.cmake`'s own R8 auto-discovery (found while fixing the NDK issue, matching its two sibling blocks' already-correct natural-sort pattern); `gradlew` losing its executable bit when committed from Windows; `KonativeCompileKotlinDex.cmake`'s kotlinc `-classpath` string hardcoding `;` (correct on Windows, wrong on Linux, where the JVM expects `:` — every android/androidx import failed as "unresolved reference" until fixed); and the same file's aapt2 target-API-level regex only accepting a plain `android-<N>` platform directory, not a real `android-<N>.<M>` preview-SDK one the runner's own pre-installed SDK offered. Build-only by design (confirms a real `.so` links, verified via `file`) — the on-device gap this left is closed by the sibling job below |
| Real Android CI, emulator-verify leg (`.github/workflows/android-build.yml`, `android-emulator-verify` job) | **Landed** — closes the build leg's own long-standing "no on-device/rendering check" gap. Builds the real `testapp/` debug APK via Gradle (the same documented recipe `testapp/README.md` gives a human), boots a real hardware-accelerated emulator (KVM + `reactivecircus/android-emulator-runner`, API 34 x86_64 — matching the already-verified local LDPlayer environment), installs and launches the app, then fails the job if logcat shows a crash (`FATAL EXCEPTION`/process death) or if any of `jni_onload.cpp`'s real `on_started()` self-checks (Taskflow, GLM+EnTT storage, `BS::thread_pool`, SPSC event queue ×2, EnTT snapshot+cereal, Boost.PFR auto-registration, Glaze JSON, `spatial::Transform` — 9 as of this writing) log `FAILED` instead of `PASSED`, asserting "at least 8" rather than an exact count specifically so a new self-check like this one doesn't need a CI-file edit to land. This is the automated equivalent of the manual on-device verification loop `testapp/README.md` documents, and specifically the kind of check that would have caught the rotation-crash bug (see the "Landed this session" write-up in the `project-konative-autonomous-loop` memory entry / git history) the moment it shipped instead of after a full session of undetected regressions — logcat + a screenshot are uploaded as job artifacts either way, for a failure to be diagnosed without needing a physical device |
| `spatial::Transform` + `to_matrix()` (`include/konative/spatial/transform.hpp`) | **Landed and self-checked** — `ARCHITECTURE.md` §4's own dependency-table framing for GLM ("Math for ECS-side transforms/components") named this as the actual reason GLM was chosen; `ecs/glm_storage_self_check.hpp`'s own `PackedTransformSelfCheckComponent` was always a synthetic, self-check-only stand-in, never a real, reusable component. `Transform` is a plain aggregate (`glm::vec3 position`, `glm::quat rotation`, `glm::vec3 scale`, standard Unity/Godot/Bevy-shaped layout, deliberately no hierarchy — see `spatial/README.md`'s own Hard Rules for why), `to_matrix()` composes them into a real 4×4 model matrix (`T * R * S` order). `transform_self_check.hpp` verifies both a real EnTT storage round-trip (the first real exercise of `glm::quat` through paged component storage, not just `vec3`) and `to_matrix()`'s actual mathematical correctness against hand-computed expected results (identity, pure translation, pure scale, a real 90°-around-Y rotation, and a composed case that specifically catches a wrong composition order) — not just "it compiles and returns a `mat4`". Desktop-tested directly (`tests/test_transform.cpp`, 53/53 suite passing) and wired into `jni_onload.cpp`'s real `on_started()` self-check chain. **Update**: the "no rendering or physics consumer yet" caveat this row used to carry here is closed — see the follower-dot row below for the real, on-device-verified consumer |
| File-backed `AppConfig` with live hot-reload (`include/konative/app/config/json_config_file.hpp`, `include/konative/jni/files_dir.hpp`) | **Landed and verified on the real emulator, end to end** — closes the `AppConfig` row's own "compiled-in JSON literal" caveat and makes `KonativeDependencies.cmake`'s stated reason for choosing Glaze ("config/hot-reload") real. First run PROVISIONS `<filesDir>/konative_config.json` from the struct defaults through the reflection WRITE path (`meta_component_to_json()`'s first production call site); later runs load it; `on_tick()` polls its mtime every 120 ticks (real measured latency, not assumed — roughly 1s on the physical phone's real 120Hz, faster still on the emulator's real ~238Hz; see `jni_onload.cpp`'s own `kConfigPollIntervalTicks` comment for the full measurement, which replaced this row's own previous "~2s at 60Hz" guess) and applies edits LIVE. Verified: a live edit moved the tick-summary cadence 120→60→240 frames matching each edit exactly, a broken edit logged once and kept previous values, a fixing edit recovered live, values persist across restarts. Full write-up in §9; two hardening findings landed with it (`clamp_to_valid()`'s division-by-zero guard, `meta_component_from_json()` rejecting non-object top-level JSON) |
| Durable ECS state persistence (`include/konative/ecs/snapshot_file.hpp` + restore-on-startup in `jni_onload.cpp`) | **Landed and verified on the real emulator, exact-math confirmed** — closes the `SnapshotSavedEvent` row's honest gap: the periodic snapshot serialized real ECS state and then THREW THE BYTES AWAY (only their count ever crossed the event), and `entt::snapshot_loader` had never restored anything outside self-check scratch registries. The existing detached background thread now streams each snapshot to `<filesDir>/konative_state.bin` atomically (temp-file-then-rename, an 8-byte magic+layout-version header guarding stale-layout files — cereal's binary archive is not self-describing), and `on_started()` restores from it before creating anything (skipping fresh entity creation when restore succeeds; `read_snapshot_file`'s clear-on-failure contract means a corrupt file degrades to a clean fresh start, never a half-restored registry). Verified across a real process kill: old process died at 10800 combined `HeartbeatCounter` ticks, new process logged `RESTORED 3 entities (10800 combined ... ticks carry over)` and its first summary read exactly 11520 = 10800 + 3×240 new frames. `SnapshotSavedEvent` gained a `persisted_to_disk` field so the event's "Saved" name is finally literal |
| `spatial::Transform`'s first real consumer — the follower dot (`PointerFollow` + `move_followers_toward_targets` in `jni_onload.cpp`, `spatial/approach.hpp`, `nativeGetFollowerX/Y`, a real Compose circle) | **Landed and verified on the real emulator, screenshot + exact-coordinate confirmed** — closes the `spatial::Transform` row's own "no rendering or physics consumer yet" caveat, and gives THREE other mechanisms their first real effect at once: touch events now AIM something (until now they only incremented a counter — a tap at Box-local (400, 700) logged `TouchDownEvent x=400.0 y=700.0` and the dot glided to exactly (400, 700), screenshot-verified), `WindowResizedEvent` now centers the initial target (a fresh launch shows the dot gliding from origin to the C++-computed center — (960, 463) for the 1920×926 window, exact), and the durable snapshot now carries real spatial state (`transform_serialize.hpp` + `PointerFollow`'s ADL `serialize()`, `kAppStateVersion` bumped 1→2 — which also gave the version header its first real-world exercise: the emulator's existing v1 file was rejected as `kVersionMismatch` and cleanly replaced, never mis-parsed; after a process kill the new process logged `follower entity RESTORED at position (400.0, 700.0)` and the dot rendered there with zero touches). The per-entity math (`approach()`: exponential smoothing, exactly frame-rate independent, never overshoots — all desktop-tested) lives in `spatial/` as a pure operation on `Transform` data; only the registry-iterating System shell is app code — the data-vs-system split `spatial/README.md` draws. The Kotlin side queries the live position per frame in `FrameTicker.doFrame()` and places the dot via `Modifier.offset`'s density-lambda overload (javap-verified against the vendored `foundation-layout.jar` before use, like every Compose API here) |
