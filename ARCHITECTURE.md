...
| C++ ECS/events core (`World`/`Application`) actually running in the real app, driven by real Android Activity lifecycle | **Landed and verified on real hardware** — previously buildable since the first commit but never instantiated anywhere outside desktop tests/examples (the "real open item, not yet decided" `include/konative/app/entry_point.hpp` and `detail/lifecycle_bridge.hpp` both flagged). `jni_onload.cpp` now implements `create_application()` and binds `KonativeEntryPoint.nativeDispatchLifecycle(Int)` via `RegisterNatives`, called from the embedded dex's existing `ActivityLifecycleCallbacks` on each of the 4 transitions `Application` models; confirmed via real logcat across a full start→resume→pause→resume→destroy session, plus new desktop unit coverage (`tests/test_app.cpp`). `World::tick()` remains undriven — a separate, still-open design question (no natural per-frame driver now that Compose owns frame scheduling), not silently forgotten |
